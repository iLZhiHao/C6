// Character 05 5.16 running.c
// running.c -- A useful program for funners
#include <stdio.h>
...
int main(void)
{
    ...
    int time;                       // 跑步用时，以秒为单位
    double mtime;                   // 跑 1 英里所用时间，以秒为单位
    int mmin,msec;                  // 跑 1 英里所用过的时间，以分钟和秒为单位
    ...
    printf("Begin by entering the mminutes.\n");
    scanf("%d",&min);
    ...
    mtime = (double) time / distm;  // 时间/距离 = 跑 1 英里所用时间
